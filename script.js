...
window.addEventListener('DOMContentLoaded', () => {
    ...
    // Menampilkan setiap catatan dalam data dummy
    function renderNotes() {
        ...
        notesData.forEach(note => {
            ...
            noteElement.innerHTML = `
                ...
                <button class="delete-title-btn" data-id="${note.id}">Delete Title</button>
            `;
    ...
    addNoteForm.addEventListener('submit', function(event) {
        ...
        const id = 'notes-' + Math.random().toString(36).substr(2, 10);

        // Tambahkan catatan baru ke dalam data dummy
        notesData.push({
            id: id,
            title: judul,
            body: isi,
    ...
    notesContainer.addEventListener('click', function(event) {
        if (event.target.classList.contains('delete-btn')) {
            ...
            // Hapus catatan dari data dummy
            notesData = notesData.filter(note => note.id !== id);

            // Render ulang daftar catatan
            renderNotes();
        } else if (event.target.classList.contains('delete-title-btn')) {
            const id = event.target.getAttribute('data-id');
        ...
        // Hapus catatan dari data dummy berdasarkan ID
        notesData = notesData.filter(note => note.id !== id);
